...
var FileProcessor = (function () {
  ...
  function cleanupPreviousRun() {
    ...
    try {
      ...
      var logs = folder.getFiles("../log/convert_*.log");
  ...
  function run() {
    ...
    Config.userSettings.maxFileSize *= 1024; // КБ → Бты
    Config.userSettings.minFileSize *= 1024;
    ...
    if (!folder.exists) {
      Utils.log("[ERROR] Путь не найден: " + Config.userSettings.sourcePath);
      Utils.closeLogFile();
      return false;
    }
    ...
    Utils.log(
      "Целевой размер: " +
        Utils.formatSize(Config.userSettings.minFileSize) +
        " — " +
